...
testRunner.component('participantDetailEdit', {
  ...
  controller: function controller($scope, participants, notif, auth, transFilter) {
    $scope.genders = ["Male", "Female", "Not determined", "Neither Male nor Female", "Prefer not to report"];
    $scope.countries = [
    ...
    $scope.sensoryIntegrationConcerns = [
      'Sensory Integration and Praxis Tests (SIPT)',
      'Sensory Processing Measure (SPM), Sensory Profile (SP), or other questionnaire',
      'Clinical Observation',
      'Other Methods'
    ];
    $scope.assessmentReasonsResearchProject = 'Research project';
    $scope.assessmentReasons = [
      'Clinical assessment',
      'As part of tester\'s SI Certificate',
      $scope.assessmentReasonsResearchProject
    ];
    $scope.clinicalAssessmentReferrers = [
      'Parent',
      'Physician',
      'Teacher',
      'Other'
    ...
    this.$onInit = function () {
      ...
      $scope.diagnosesSelected = JSON.parse(this.participant.diagnosesSelected);
      $scope.researchProjectSelected = JSON.parse(this.participant.researchProjectSelected);
      $scope.admin = auth.user

      if (this.participant.dateOfBirth) {
        $scope.dateOfBirth = new Date(this.participant.dateOfBirth);
      } else {
        $scope.dateOfBirth = null;
      }
    }
    ...
    $scope.save = function () {
      let participant = Object.assign($scope.participant, {
        dateOfBirth: $scope.dateOfBirth.getFullYear() + "-" + (($scope.dateOfBirth.getMonth() + 1).toString().padStart(2, '0')) + "-" + $scope.dateOfBirth.getDate().toString().padStart(2, '0'),
