...
responses = concerto.table.query("SELECT * FROM {{responsesTable}} WHERE session_id='{{id}}'", list(responsesTable=responsesTable, id=session$id))

#scores
scoresTable = paste0(test$code, "_scores")
scoresRecords = concerto.table.query("SELECT * FROM {{scoresTable}} WHERE session_id='{{id}}'", list(scoresTable=scoresTable, id=session$id))
scores = list()
...
(maxParticipantMonths>='{{months}}' OR maxParticipantMonths IS NULL)", list(settingsTable=settingsTable, months=session$participantMonths))
...
#default settings
if(is.na(settings$itemsperpage)) { settings$itemsperpage = nrow(items) }
