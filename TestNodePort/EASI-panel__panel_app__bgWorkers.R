cognitoClientId = "6bbto14uknbbc4p2va7uvih8ac"
  ...
  if (TRUE) {
    cognito = tryCatch({
      ...
      if (status_code(response) == 200) {
        ...
        expirationDate = getElement(decodedJson$IdToken[[2]], 'custom:expirationDate')
        ...
        highest_degree = getElement(decodedJson$IdToken[[2]], 'custom:highest_degree') # TODO: Normalize to camel case
        if (is.null(type)) {
        ...
        admins = concerto.table.query("SELECT * FROM EASI_admins WHERE login='{{login}}' AND enabled=1", list(login = email))
        if (nrow(admins) > 0) {
          ...
          for (i in 1:nrow(admins)) {
            ...
            concerto.table.query("UPDATE EASI_admins SET type='{{type}}', enabled='{{enabled}}', researchGroup='{{researchGroup}}', email='{{email}}', cohort='{{cohort}}', expirationDate='{{expirationDate}}', profession='{{profession}}', country='{{country}}', usergroup='{{usergroup}}', gender='{{gender}}', name='{{name}}' WHERE id='{{id}}'", params)
          }
        }
        else {
          # User not in table
          ...
          newUsers = concerto.table.query("INSERT INTO EASI_admins SET login='{{login}}', type='{{type}}', enabled='{{enabled}}', researchGroup='{{researchGroup}}', email='{{email}}', cohort='{{cohort}}', expirationDate='{{expirationDate}}', profession='{{profession}}', country='{{country}}', usergroup='{{usergroup}}', gender='{{gender}}', name='{{name}}'", params)
        }
        admins = concerto.table.query("SELECT * FROM EASI_admins WHERE login='{{login}}' AND enabled=1", list(login = email))
        if (nrow(admins) > 0) {
          for (i in 1:nrow(admins)) {
        ...
        paste("Status code is: ", status_code(response), "\n Response is: ", content(response))
        return(list(
          id = -1,
    ...
    admins = concerto.table.query("SELECT * FROM EASI_admins WHERE login='{{login}}' AND enabled=1", list(login = login))
    if (nrow(admins) > 0) {
      for (i in 1:nrow(admins)) {
        admin = admins[i,]
        if (!is.na(admin$algo)) {
  ...
  if (status_code(response) == 200) {
    ...
    if (nrow(admins) > 0) {
      ...
      for (i in 1:nrow(admins)) {
        ...
        admins = concerto.table.query("SELECT * FROM EASI_admins WHERE id='{{id}}'", list(id = id))
        if (nrow(admins) > 0) {
          for (i in 1:nrow(admins)) {
            admin = admins[i,]
            id = admin$id # Sets admin id for return
    ...
    # refresh token not valid, logging error
    returnValue = list(
      user = user,
      token = token,
      error = toString(content(response, as = "text"))
    )
  }
}
...
updateUserProfile = function(user, token) {
  ...
  if (status_code(response) == 200) {
    ...
    for (Attribute in UserAttributes) {
      ...
      if (status_code(response) != 200) {
        error[[Attribute$Name]] = list(Attribute = Attribute, body = body, status_code = status_code(response))
      }
    }
    ...
    concerto.table.query("UPDATE EASI_admins SET type='{{type}}', researchGroup='{{researchGroup}}', highestDegree='{{highestDegree}}', country='{{country}}', usergroup='{{usergroup}}', email='{{email}}', cohort='{{cohort}}', expirationDate='{{expirationDate}}', profession='{{profession}}', gender='{{gender}}', name='{{name}}' WHERE id='{{id}}'", params)
    admins = concerto.table.query("SELECT * FROM EASI_admins WHERE id='{{id}}' AND enabled=1", list(id = id))
    if (nrow(admins) > 0) {
      for (i in 1:nrow(admins)) {
        admin = admins[i,]
        user = list(
...
cognitoLogInCode = function(code) {
  ...
  library(caTools)
  returnValue = list()
  body = paste0("grant_type=authorization_code&code=", code, "&client_id=", cognitoClientId, "&redirect_uri=https%3A%2F%2Fwww.easitests.com%2Ftest%2Fpanel")
  response <- POST(
...
decodeJwt = function(rawJwt) {
  ...
  library(openssl)
  library(caTools)
  splitRawJwt = list(# Split jwt at periods
    strsplit(rawJwt, "\\.")[[1]][1],
    strsplit(rawJwt, "\\.")[[1]][2]
  )
  ...
  # %% = 2 -> 2
  # %% = 3 -> 1
  modNum = list(# number of equals to pad. Needs to be multiple of 4 characters, hence the modulus 4
    if ((nchar(splitRawJwt[[1]]) %% 4) == 0) { 0 } else {(4 - (nchar(splitRawJwt[[1]]) %% 4)) },
    if ((nchar(splitRawJwt[[2]]) %% 4) == 0) { 0 } else {(4 - (nchar(splitRawJwt[[2]]) %% 4)) }
  )
  splitRawPaddedJwt = list(# padding equals using modNum
    paste0(
      splitRawJwt[[1]],
      strrep("=", modNum[[1]])
    ),
...
getOrderSql = function(order, validColumns) {
  ...
  if (!order %in% validColumns) { order = validColumns[1] }
  paste0(order, " ", orderDir)
}

getSearchSql = function(query, columns) {
  #search
  searchSql = "1"
  if (!is.null(query$search) && query$search != "") {
    searchSql = paste0(columns, " LIKE '%{{search}}%'", collapse = " OR ")
    searchSql = paste0("(", searchSql, ")")
  }

  #archived
  archivedSql = "1"
  if (!is.null(query$archived) && query$archived != "") {
    archivedSql = "archived='{{archived}}'"
  }

  concerto.table.insertParams(paste0(searchSql, " AND ", archivedSql), list(
    search = query$search,
...
getLimitSql = function(query) {
  concerto.log(query)
  if (is.null(query$limit) || query$limit == "") { return("") }
...
adminFetchParticipants = function(query, admin) {
  ...
  searchSql = getSearchSql(query, columns)
...
ORDER BY {{orderSql}} {{limitSql}}")
    totalCountSql = paste0("SELECT COUNT(*) FROM EASI_participants AS p WHERE ", searchSql)
  }
  if (admin$type == 2) {
    collectionSql = paste0("
SELECT p.*, a.login AS adminLogin FROM EASI_participants AS p
LEFT JOIN EASI_admins AS a ON a.id=p.admin_id
WHERE (", searchSql, ") AND (admin_id='{{admin_id}}' OR p.researchGroup='{{admin_researchGroup}}')
ORDER BY {{orderSql}} {{limitSql}}")
    totalCountSql = paste0("SELECT COUNT(*) FROM EASI_participants AS p WHERE (", searchSql, ") AND (admin_id='{{admin_id}}' OR researchGroup='{{admin_researchGroup}}')")
  }
  if (admin$type == 0) {
    collectionSql = paste0("
SELECT p.*, a.login AS adminLogin FROM EASI_participants AS p
LEFT JOIN EASI_admins AS a ON a.id=p.admin_id
WHERE (", searchSql, ") AND admin_id='{{admin_id}}' 
ORDER BY {{orderSql}} {{limitSql}}")
    totalCountSql = paste0("SELECT COUNT(*) FROM EASI_participants AS p WHERE (", searchSql, ") AND admin_id='{{admin_id}}'")
  }
...
fetchSingleParticipant = function(id) {
  ...
  if (admin$type == 2) {
    sql = paste0("SELECT * FROM EASI_participants WHERE id='{{id}}' AND (admin_id='{{admin_id}}' OR researchGroup='{{admin_researchGroup}}')")
  }
  if (admin$type == 0) {
    sql = paste0("SELECT * FROM EASI_participants WHERE id='{{id}}' AND admin_id='{{admin_id}}'")
  }
  participant = concerto.table.query(sql, list(
    id = id,
    admin_id = admin$id,
    admin_researchGroup = admin$researchGroup
  ))
...
deleteParticipants = function(ids) {
  ...
  if (admin$type == 1) {
    concerto.table.query("DELETE FROM EASI_participants WHERE id IN ({{ids}})", params)
  }
  if (admin$type == 2) {
    concerto.table.query("DELETE FROM EASI_participants WHERE id IN ({{ids}}) AND (admin_id='{{admin_id}}' OR researchGroup='{{admin_researchGroup}}')", params)
  }
  if (admin$type == 0) {
    concerto.table.query("DELETE FROM EASI_participants WHERE id IN ({{ids}}) AND admin_id='{{admin_id}}'", params)
  }
}
...
toggleArchivedParticipants = function(ids) {
  ...
  if (admin$type == 1) {
    concerto.table.query("UPDATE EASI_participants SET archived=ABS(archived-1) WHERE id IN ({{ids}})", params)
  }
  if (admin$type == 2) {
    concerto.table.query("UPDATE EASI_participants SET archived=ABS(archived-1) WHERE id IN ({{ids}}) AND (admin_id='{{admin_id}}' OR researchGroup='{{admin_researchGroup}}')", params)
  }
  if (admin$type == 0) {
    concerto.table.query("UPDATE EASI_participants SET archived=ABS(archived-1) WHERE id IN ({{ids}}) AND admin_id='{{admin_id}}'", params)
  }
}
...
generateRandomId = function() {
  ...
  while (T) {
    ...
    result = concerto.table.query("SELECT COUNT(*) FROM EASI_participants WHERE customId='{{id}}'", list(id = id))
    if (result[1, 1] == 0) { break }
  }

  id
}

downloadParticipants = function(ids) {
  admin = c.get("admin", T)
  if (!is.list(admin)) return(NULL)
  ...
  participantsSql = "SELECT id, customId, dateOfBirth, countryOfResidence, gender, languageCode, diagnoses, diagnosesSelected, initials, email, assessmentReason, clinicalAssessmentReferrer, researchProjectSelected, researchGroup 
FROM EASI_participants
"

  if (admin$type == 1) {
    participantsSql = paste0(participantsSql, " WHERE id IN ({{ids}})")
    participants = concerto.table.query(participantsSql, params)
  }
  if (admin$type == 2) {
    participantsSql = paste0(participantsSql, " WHERE id IN ({{ids}}) AND (admin_id='{{admin_id}}' OR researchGroup='{{admin_researchGroup}}')")
    participants = concerto.table.query(participantsSql, params)
  }
  if (admin$type == 0) {
    participantsSql = paste0(participantsSql, " WHERE id IN ({{ids}}) AND admin_id='{{admin_id}}'")
    participants = concerto.table.query(participantsSql, params)
  }
...
WHERE se.participant_id IN ({{ids}}) AND session_id = (SELECT MAX(session_id) FROM ", testCodes, "_responses LEFT JOIN ", testCodes, "_sessions as ses on ses.id=session_id WHERE ses.participant_id=se.participant_id)")
  ...
  dateUTC <- format(Sys.time(), "%Y%m%dT%H%M%SZ", tz="GMT")
  filename = paste0("participants_", dateUTC, ".csv")
  write.csv(result, paste0("files/", filename), row.names = F, na = "")
...
", params)
  ...
  as.list(concerto.table.query("SELECT * FROM EASI_participants WHERE id='{{id}}'", list(id = concerto.table.lastInsertId())))
}
...
saveParticipant = function(newParticipant) {
  ...
  currentParticipant = concerto.table.query("SELECT * FROM EASI_participants WHERE id='{{id}}'", newParticipant)
  if (
    nrow(currentParticipant) == 0 ||
    (admin$type == 0 && currentParticipant$admin_id != admin$id) ||
    (admin$type == 2 && currentParticipant$admin_id != admin$id && (is.na(currentParticipant$researchGroup) || is.na(admin$researchGroup) || currentParticipant$researchGroup != admin$researchGroup))
  ) {
    #not authorized
    concerto.log(paste0("not authorized to access participant id: ", currentParticipant$id))
    return(NULL)
  }
...
WHERE id='{{id}}'", params)
  ...
  newParticipant = as.list(concerto.table.query("SELECT * FROM EASI_participants WHERE id='{{id}}'", list(id = newParticipant$id)))
  ...
  if (currentParticipant$valid == 0) {
    ...
    if(!is.null(params$autoDemographicsInvite) && params$autoDemographicsInvite == 1) {
    ...
    for (i in 1:nrow(tests)) {
      ...
      if (nrow(participants) > 0) {
        for (j in 1:nrow(participants)) {
          ...
          concerto.table.query("INSERT INTO {{testCode}}_sessions SET participant_id='{{participant_id}}', timeCreated=NOW(), token='{{token}}'", list(
            testCode = test$code,
...
WHERE session.id='{{id}}'", list(
  ...
  id = concerto.table.lastInsertId()
          ...
          if (test$autoInvitationEmail == 1) {
            autoInvitationSessions = rbind(autoInvitationSessions, data.frame(session, stringsAsFactors = F))
          }
        }
...
fetchAllTests = function() {
  ...
  titleTransCol = concerto$globals$easi$lib$getTransCol("EASI_tests", "title", language)
  tests = concerto.table.query("
SELECT 
*,
IFNULL({{titleTransCol}}, title) title_trans
FROM EASI_tests ORDER BY code ASC", list(titleTransCol=titleTransCol))
  tests
}
...
IFNULL({{titleTransCol}}, title) title_trans
FROM EASI_tests", list(titleTransCol=titleTransCol))
...
addSession = function(session) {
  ...
  if (
    ...
    (admin$type == 2 && participant$admin_id != admin$id && (is.na(participant$researchGroup) || is.na(admin$researchGroup) || participant$researchGroup != admin$researchGroup))
  ) {
    concerto.log(paste0("not authorized to access participant id: ", participant$id))
    return(NULL)
  }

  #test check
  test = concerto.table.query("SELECT * FROM EASI_tests WHERE code='{{testCode}}'", list(testCode = session$testCode))
  if (nrow(test) == 0) {
...
deleteSessions = function(ids) {
  ...
  for (testCode in ls(ids)) {
    #test check
    test = concerto.table.query("SELECT * FROM EASI_tests WHERE code='{{testCode}}'", list(testCode = testCode))
    if (nrow(test) == 0) {
...
hiddenScores 
FROM EASI_tests
ORDER BY orderIndex ASC, title ASC", list(titleTransCol=titleTransCol))
  filteredTests = NULL
  ...
  for (i in 1:nrow(tests)) {
    ...
    if (query$allTests == 0 && (!is.list(query$tests) || !test$id %in% ls(query$tests) || query$tests[[as.character(test$id)]] != 1)) { next }
    filteredTests = rbind(filteredTests, test)

    feedbackTable = paste0(test$code, "_feedback")
    feedbackTransCol = concerto$globals$easi$lib$getTransCol(feedbackTable, "feedback", language)
    params = list(
...
"
  ...
  if (testsSelected) {
    sqlCollection = paste0("
SELECT *, UNIX_TIMESTAMP(timeCreated) AS timestamp FROM (", paste0(sqlArray, collapse = " UNION "), ") AS tu
...
fetchSessionScores = function(testCode, sessionId) {
  ...
  test = concerto.table.query("SELECT * FROM EASI_tests WHERE code='{{testCode}}'", list(testCode = testCode))
  if (nrow(test) == 0) { stop("invalid test") }
...
fetchSessionResponses = function(testCode, sessionId) {
  ...
  auditLabelTransCol = concerto$globals$easi$lib$getTransCol(itemTable, "auditLabel", language)
  optionLabel1TransCol = concerto$globals$easi$lib$getTransCol(itemTable, "optionLabel1", language)
  optionLabel2TransCol = concerto$globals$easi$lib$getTransCol(itemTable, "optionLabel2", language)
  optionLabel3TransCol = concerto$globals$easi$lib$getTransCol(itemTable, "optionLabel3", language)
  optionLabel4TransCol = concerto$globals$easi$lib$getTransCol(itemTable, "optionLabel4", language)
  optionLabel5TransCol = concerto$globals$easi$lib$getTransCol(itemTable, "optionLabel5", language)
...
fetchDemographics = function(participantId) {
  ...
  labelTransCol = concerto$globals$easi$lib$getTransCol("EASI_demographics_fields", "label", language)
...
sendSessionEmail = function(session) {
  ...
  concerto.test.run("EASI-email-invitation", list(
    sessions = session
  ))
}
...
importParticipant = function(params) {
  ...
  csv = tryCatch({ read.csv("import.csv") }, error = function(e) { NULL })
  if (is.null(csv)) {
    return(list(messages = list(c.trans("csv_client_validation_invalid")), num = 0))
  }
  ...
  if (nrow(csv) > 0) {
    for (i in 1:nrow(csv)) {
      ...
      if (!valid) {
        msg = paste0(c.trans("csv_client_validation_invalid_language_code"), i + 1)
        response$messages = append(response$messages, msg)
        success = F
      }
      ...
      if (!valid) {
        msg = paste0(c.trans("csv_client_validation_invalid_email_length"), i + 1)
        response$messages = append(response$messages, msg)
        success = F
      }
      ...
      if (!valid) {
        msg = paste0(c.trans("csv_client_validation_invalid_diagnoses_selected"), i + 1)
        response$messages = append(response$messages, msg)
        success = F
      }
      ...
      if (!valid) {
        msg = paste0(c.trans("csv_client_validation_invalid_assessment_reason"), i + 1)
        response$messages = append(response$messages, msg)
        success = F
      }
      ...
      if (!valid) {
        msg = paste0(c.trans("csv_client_validation_clinical_assessment_referrer"), i + 1)
        response$messages = append(response$messages, msg)
        success = F
      }
      ...
      if (!valid) {
        msg = paste0(c.trans("csv_client_validation_research_project_selected"), i + 1)
        response$messages = append(response$messages, msg)
        success = F
      }
      ...
      if (!valid) {
        msg = paste0(c.trans("csv_client_validation_invalid_auto_demographics_invite"), i + 1)
        response$messages = append(response$messages, msg)
        success = F
      }
    }
...
sendParentEmail = function(participants) {
  ...
  for(languageCode in languageCodes) {
    if(is.list(participants)) { 
      ...
      participantsSubset = participants[participants$languageCode == languageCode,]
    }

    concerto.test.run("EASI-email-demographics", list(
...
createLoginToken = function(admin, token) {
  if (!is.na(admin)) {
    ...
    repeat {
      ...
      result = concerto.table.query("SELECT * FROM EASI_admin_tokens WHERE token='{{token}}'", list(token = token))
      if (nrow(result) == 0) { break }
    }
    concerto.table.query("INSERT INTO EASI_admin_tokens SET admin_id='{{id}}', token='{{token}}', expiryTime=DATE_ADD(NOW(), INTERVAL 3 HOUR)", list(id = admin$id, token = token))
  }
  token
}
...
getAdminFromToken = function(token) {
  ...
  result = concerto.table.query("SELECT * FROM EASI_admin_tokens WHERE token='{{token}}' AND expiryTime > NOW()", list(token = token))
  if (nrow(result) == 0) {
    NA
  } else {
    concerto.table.query("UPDATE EASI_admin_tokens SET expiryTime=DATE_ADD(NOW(), INTERVAL 3 HOUR) WHERE id='{{id}}'", list(id = result$id))
...
highestDegree,
languageCode
FROM EASI_admins WHERE id='{{id}}'", list(id = result$admin_id)))
  }
}

getTherapists = function() {
  admins = concerto.table.query("SELECT * FROM EASI_admins WHERE type=0 AND enabled=1")
}
...
setAdminLanguage = function(languageCode) {
  ...
  concerto.table.query("UPDATE EASI_admins SET languageCode='{{languageCode}}' WHERE id='{{id}}'", list(id=admin$id, languageCode=languageCode))
}
...
list(
  ...
  downloadParticipants = function(response) {
    list(filename = downloadParticipants(response$ids))
  },
