  ...
  tryCatch(
    {
      ...
      list(
        ...
        totalCount = totalCountResult[1, 1]
      )
    },
    error = function(err) {
      concerto.log(paste("FetchParticipants error:", err$message))
      list(
        success = FALSE,
        error = err$message,
        collection = list(),
...
getAdminPermissionSql <- function(admin, alias = "") {
  ...
  if (admin$type == 2) {
    # Group admin: own records OR records in the same research group
    return(paste0(
      " AND (",
      prefix, "admin_id = ", admin$id,
      " OR ",
      prefix, "researchGroup = '", admin$researchGroup, "')"
    ))
  }
    ...
    for (i in 1:nrow(tests)) {
      ...
      if (nrow(participants) > 0) {
        for (j in 1:nrow(participants)) {
          ...
          concerto.table.query(
            "INSERT INTO {{testCode}}_sessions
          ...
          session <- as.list(concerto.table.query(
            "
SELECT session.*, participant.email, testIteration, participant.languageCode, '{{testCode}}' AS testCode, '{{testId}}' AS testId
...
fetchTestSessions <- function(query) {
  admin <- c.get("admin", T)
  if (!is.list(admin)) {
    return(NULL)
  }
  concerto.log(
    jsonlite::toJSON(query, pretty = TRUE, auto_unbox = TRUE)
  )
  sessions <- concerto.table.query(
    "SELECT * from {{testCode}}_sessions where participant_id = '{{participantId}}'",
    list(testCode = query$testCode, participantId = query$participantId)
  )
  collection <- sessions
}
...
fetchSessions <- function(query) {
  ...
  limitSql <- getLimitSql(query)
  adminSql <- getAdminPermissionSql(admin, "p")
  ...
  if (query$testCode != "*") {
    testFilterSql <- "WHERE code = '{{testCode}}'"
  }
  ...
  for (i in seq_len(nrow(tests))) {
    ...
    sqlTest <- "
        ...
        INNER JOIN EASI_participants AS p ON p.id=s.participant_id
    ...
    SELECT
      ...
      '{{testCode}}' AS testCode
    FROM {{sessionTable}} AS s
    INNER JOIN EASI_participants AS p
      ON p.id = s.participant_id
    WHERE s.participant_id = '{{participant_id}}'
    {{adminSql}}"
    sqlTestCount <- concerto.table.insertParams(sqlTestCount, params)
    sqlTest <- concerto.table.insertParams(sqlTest, params)
    sqlArray <- c(sqlArray, sqlTest)
    countSqlArray <- c(countSqlArray, sqlTestCount)
  }
  ...
  sqlTotalCount <- paste0(
    ...
    FROM (
    ",
    paste0(countSqlArray, collapse = " UNION ALL "),
    "
    ) AS tu"
  )
...
addSession <- function(session) {
  ...
  session <- as.list(concerto.table.query(
    "SELECT session.*, participant.email, participant.languageCode,
    '{{testCode}}' AS testCode, '{{testId}}' AS testId
    FROM {{testCode}}_sessions AS session
    LEFT JOIN EASI_participants AS participant ON participant.id=session.participant_id
    WHERE session.id='{{id}}'",
    list(
...
fetchScores <- function(query) {
  ...
  participantQuery <- paste0("
    SELECT p.id
    FROM EASI_participants AS p
    LEFT JOIN EASI_admins AS a
      ON a.id = p.admin_id
    WHERE p.id = {{id}}", permissionSql)
...
list(
  ...
  fetchTestSessions = function(response) {
    result <- fetchTestSessions(response)
  },
