...
generate = function(ids, cols, admin, filePath) {
  ...
  if (admin$type == 0) {
    participantsSql = paste0(
      participantsSql,
      " WHERE exportExclusion=0 AND id IN ({{ids}}) AND admin_id='{{admin_id}}'"
    )
  }
  if (admin$type == 1) {
    participantsSql = paste0(
      participantsSql,
      " WHERE exportExclusion=0 AND id IN ({{ids}})"
    )
  }
  if (admin$type == 2) {
    participantsSql = paste0(
      participantsSql,
      " WHERE exportExclusion=0 AND id IN ({{ids}}) AND (admin_id='{{admin_id}}' OR researchGroup='{{admin_researchGroup}}')"
    )
  }
...
queue = concerto.table.query(
  "SELECT * FROM EASI_export_queue WHERE status=0 LIMIT {{batchSize}}",
  list(batchSize = batchSize)
)
if (nrow(queue) > 0) {
  concerto.table.query(
    "UPDATE EASI_export_queue SET status=1 WHERE id IN ({{ids}})",
    list(ids = paste0(queue$id, collapse = ","))
  )
  ...
  for (i in 1:nrow(queue)) {
    ...
    concerto.table.query(
      "UPDATE EASI_export_queue SET status='{{status}}' WHERE id='{{id}}'",
      list(id = export$id, status = status)
    )
  }
}
